...
# --- 1. INITIALIZE API & NLP ENGINE ---
app = FastAPI(
    title="ASTRAM-AI V8.2 TITAN",
    description="Zero-Touch Multi-Modal Probabilistic Grid",
    version="8.2.0"
)
print("Initializing Geospatial Grid with 1,000 Dispatch Nodes...")
random.seed(42)
...
print("Loading V8 System Assets...")
try:
    ...
    print(f"Startup Warning: Missing artifacts. Run train.py first! Error: {e}")

print("Loading NLP HuggingFace Transformer...")
nlp_model = SentenceTransformer('all-MiniLM-L6-v2')


# --- 2. V8.2 TITAN NEURAL ARCHITECTURE REBUILD ---
...
class AgenticLLMNode:
    def analyze(self, text: str):
        ...
        context = {
            ...
            "force_road_closure": False # NEW FAILSAFE
        }
...
async def predict_event(event: TrafficEventInput):
    try:
        # 1. LLM SOP & Priority Analysis
        llm_ctx = llm_node.analyze(event.description)

        text_embedding = nlp_model.encode([event.description])
        ...
        with torch.no_grad():
            ...
            # ROAD CLOSURE FIX (AI Prediction + LLM Failsafe)
        ...
        # RL RESOURCE ALLOCATOR FIX (Dynamic Priority Scaling)
        ...
        # FIX: The RL logic is now dynamic based on LLM inferred priority
        if llm_ctx["inferred_priority"] == "High":
            deployment_divisor = 8  # Needs lots of officers
        elif llm_ctx["inferred_priority"] == "Medium":
            deployment_divisor = 15 # Needs some officers
        else:
            deployment_divisor = 25 # Low priority (Two wheeler breakdown) needs very few

        # Ensure minimum of 1 officer for very low severity
